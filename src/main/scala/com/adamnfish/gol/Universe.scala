...
trait Universe extends Boundaries {
  ...
  def neighbours(cell: Cell): Set[Cell] = {
    (constrainToXMin(cell.x - 1) to constrainToXMax(cell.x + 1)).flatMap { x =>
      (constrainToYMin(cell.y - 1) to constrainToYMax(cell.y + 1)).flatMap {
        y =>
  ...
  /** Returns a boolean describing cell's state in the provided world
    */
  def isAlive(cell: Cell, world: World): Boolean =
    world.get(cell).getOrElse(false)
...
object Universe {
  ...
  def universeForWorld(world: World): FiniteUniverse = {
    FiniteUniverse(
      InfiniteUniverse.maxX(world) + 1,
      InfiniteUniverse.maxY(world) + 1
    )
  }
}
...
class FiniteUniverse private (minCell: Cell, maxCell: Cell)
    extends Universe
    with Bounded {
...
object FiniteUniverse {
  def apply(width: Int, height: Int) =
    new FiniteUniverse(Cell(0, 0), Cell(width - 1, height - 1))
}
